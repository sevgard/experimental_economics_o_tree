...
class ShuffleWaitPage(WaitPage):
    ...
    def after_all_players_arrive(self):
        if self.round_number == 1:
            ...
            A_players = [p for p in players if p.participant.vars['type'] == 'A']#p.participant.label == 'A']
            B_players = [p for p in players if p.participant.vars['type'] == 'B']#p.participant.label == 'B']
            shuffle(A_players)
            shuffle(B_players)
